...
int gf_w8_log_init(gf_t *gf)
{
  ...
  if (h->mult_type == GF_MULT_LOG_ZERO) ZeroMemory(alt+510, 255);

  if (h->mult_type == GF_MULT_LOG_ZERO_EXT) {
      ZeroMemory(alt+512, 255);
    alt[512+512] = 0;
  }
...
int gf_w8_split_init(gf_t *gf)
{
  ...
  ZeroMemory(htd->high, sizeof(uint8_t)*GF_FIELD_SIZE*GF_HALF_SIZE);
  ZeroMemory(htd->low, sizeof(uint8_t)*GF_FIELD_SIZE*GF_HALF_SIZE);
...
int gf_w8_table_init(gf_t *gf)
{
  ...
  if (h->mult_type == GF_MULT_DEFAULT &&
    ...
    scase = 3;
    ZeroMemory(dd->high, sizeof(uint8_t) * GF_FIELD_SIZE * GF_HALF_SIZE);
    ZeroMemory(dd->low, sizeof(uint8_t) * GF_FIELD_SIZE * GF_HALF_SIZE);
    ZeroMemory(dd->divtable, sizeof(uint8_t) * GF_FIELD_SIZE * GF_FIELD_SIZE);
    ZeroMemory(dd->multtable, sizeof(uint8_t) * GF_FIELD_SIZE * GF_FIELD_SIZE);
    ...
    ZeroMemory(ftd->divtable, sizeof(uint8_t) * GF_FIELD_SIZE * GF_FIELD_SIZE);
    ZeroMemory(ftd->multtable, sizeof(uint8_t) * GF_FIELD_SIZE * GF_FIELD_SIZE);
    scase = 0;
  } else if (h->region_type == GF_REGION_DOUBLE_TABLE) {
    dtd = (struct gf_w8_double_table_data *)h->private;
    ZeroMemory(dtd->div, sizeof(uint8_t) * GF_FIELD_SIZE * GF_FIELD_SIZE);
    ZeroMemory(dtd->mult, sizeof(uint16_t) * GF_FIELD_SIZE * GF_FIELD_SIZE * GF_FIELD_SIZE);
    scase = 1;
  } else if (h->region_type == (GF_REGION_DOUBLE_TABLE | GF_REGION_LAZY)) {
    ltd = (struct gf_w8_double_table_lazy_data *)h->private;
    ZeroMemory(ltd->div, sizeof(uint8_t) * GF_FIELD_SIZE * GF_FIELD_SIZE);
    ZeroMemory(ltd->smult, sizeof(uint8_t) * GF_FIELD_SIZE * GF_FIELD_SIZE);
    scase = 2;
  } else {
